...
function ViewShips({apiToken, sysSymbol, shipWaypointSymbol, setShipWaypointSymbol}) {
    ...
    const [form, setForm] = useState({ token: apiToken, systemSymbol: sysSymbol, startWaypointSymbol: shipWaypointSymbol});
    ...
    return(<>
        ...
        <input type="submit" onClick={async () => {
            const resp = await fetch("https://api.spacetraders.io/v2/systems/" + form.systemSymbol + "/waypoints/" + form.startWaypointSymbol + "/shipyard", {
            headers: {
